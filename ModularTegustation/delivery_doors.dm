...
/obj/structure/delivery_door/proc/OrderItems(origin, obj/item/T = /obj/item/food/pizza)
	if(!isturf(origin) && !isatom(origin))
		return FALSE
	item_order = list(T)
	var/obj/item/paper/P = new (get_turf(origin))
	P.setText("[address] orders a pizza.")
	return TRUE
...
/obj/item/delivery_parcel
	...
	desc = "Your attempts to open the package are foiled by a J corp security lock."
